...
export const updateTenant =async (tenant:Tenant) => {
    return execute<OkPacket>(tenantQueries.updateTenant,
        [tenant.name, tenant.profession, tenant.description, tenant.phone, tenant.image, tenant.suite, tenant.serviceOne, tenant.serviceTwo, tenant.serviceThree, tenant.tenantId]);
}
